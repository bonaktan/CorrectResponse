...
const useInput = () => {
    ...
    const sanityCheck = (e) => {
        let validity = ""
        if (!/^\d+$/.test(e.target.value)) { validity = "Number Only" }
        else if (0 >= parseInt(e.target.value)) { validity = "Out of Range" }
        else if (parseInt(e.target.value) >= Count.values.current.length + 1){ validity = "Out of Range" }
        e.target.setCustomValidity(validity)
    }
    const DisplayNavigation = (state, action) => {
        switch (action) {
            case "PMAX": state.num = 1; break
            case "PONE": state.num -= 1; break
            case "NONE": state.num += 1; break
            case "NMAX": state.num = state.max; break
    ...
    return { Mode, values, toggleMode, submitValue, sanityCheck, page, pagenav,}
}
